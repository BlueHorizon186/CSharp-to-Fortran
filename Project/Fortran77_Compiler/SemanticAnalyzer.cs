...
namespace Fortran77_Compiler
{
    public class SemanticAnalyzer
    {
        ...
        {
            foreach (var entry in GlobalTable)
            {
                var localTable = entry.Value;
                if (localTable.Contains(id))
                    return true;
            }
            return false;
        }
        ...
        public Type GetIdType(string id)
        {
            foreach (var entry in GlobalTable)
            {
                var localTable = entry.Value;
                if (localTable.Contains(id))
                    return localTable[id].SymbolType;
            }
            return Type.NONE;
        }
        ...
        //-----------------------------------------------------------
        public Type SecondRun(Program node)
        {
            Visit((Program) node);
            return Type.VOID;
        }
        ...
        public Type Visit(Program node)
        {
            ...
            currentProgUnit = programName;
            var programTable = GlobalTable[currentProgUnit];
